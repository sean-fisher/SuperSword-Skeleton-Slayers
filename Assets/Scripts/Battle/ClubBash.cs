...
public class ClubBash : Attack {
    ...
    public override IEnumerator UseAttack(BaseCharacter attacker, BaseCharacter target, List<Turn> turnList)
    {
        if (attacker)
        {
            ...
            string battleMessage = String.Format("{0} club bashes {2}, scoring {3} damage!", attacker.characterName, target.characterName, damageDealt);
