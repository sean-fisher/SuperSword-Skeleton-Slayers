...
public class ReusableAttack : Attack {
    ...
    public override IEnumerator UseAttack(BaseCharacter attacker, BaseCharacter target, List<Turn> turnList)
    {
        if (attacker)
        {
            ...
            string battleMessage = String.Format(this.display, attacker.characterName, this.attackName, target.characterName, damageDealt);
            ...
            {
                if (e.AppliesTo(attacker, target, damageDealt, this))
                {
                    e.Apply(attacker, target, damageDealt, this);
                    string effectMessage = String.Format(e.display, attacker.characterName, this.attackName, target.characterName, damageDealt);
                    TextBoxManager.tbm.EnableTextBox(battleMessageWindow, effectMessage, false);
